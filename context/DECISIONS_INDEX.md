...
| D031 | UX | Rush de países: auto-aceptación al escribir, con espera de 700 ms/Enter cuando el texto es prefijo ambiguo de otro país sin descubrir | `decisions/07-modo-paises.md` | Pendiente (Fase 4) |
| D032 | UX | Rush de países: comparación siempre sin diacríticos (`difficulty: "easy"`), aunque el resto del competitivo fuerce `"hard"` | `decisions/07-modo-paises.md` | Pendiente (Fase 4) |
| D033 | UX | Rush de países: termina completando o rindiéndose; el mejor tiempo solo se registra al 100 % (`completed`) | `decisions/07-modo-paises.md` | Pendiente (Fase 4) |
| D034 | UX | Práctica de países: tarjeta cloze sobre el tablero del continente, con pistas letra a letra | `decisions/07-modo-paises.md` | Pendiente (Fase 5) |
| D035 | Persistencia | `dailyPracticeQueue` pasa a `{ gameType, codes } \| null`: la práctica diaria es por juego | `decisions/07-modo-paises.md` | Implementado (Fase 1, `feat/modo-paises`, experimental) |
| D036 | Logros | Los logros existentes siguen siendo de Banderas; `rush_impecable`/`sin_frenos` filtran además `(session.gameType ?? "flags") === "flags"`. 4 logros nuevos de Países en la Fase 7 | `decisions/07-modo-paises.md` | Implementado (Fase 1) / Pendiente (logros nuevos, Fase 7) |
| D037 | Animación | Animación de "vuelo" (texto del input a su hueco en el tablero) con Web Animations API, no framer-motion | `decisions/07-modo-paises.md` | Pendiente (Fase 3) |
| D038 | Diseño | Ancho del hueco = ancho del nombre real (`invisible`), sin valores mágicos de width | `decisions/07-modo-paises.md` | Pendiente (Fase 3) |
